...
is_gpu_available = tf_test_util.is_gpu_available()
...
def get_conda_packages(run_lambda):
    if get_platform() == 'win32':
        grep_cmd = r'findstr /R "torch soumith tensorflow"'
    else:
        grep_cmd = r'grep "torch\|soumith\|tensorflow"'
...
def get_nccl_version(run_lambda):
    """This will return a list of libnccl.so; it's hard to tell which on eis being used"""
    if get_platform() == 'win32':
        nccl_cmd = 'where /R "%CUDA_PATH%\\bin" nccl*.dll'
    else:
        nccl_cmd = 'find /usr/local /usr/lib -type f -name "libnccl*" 2> /dev/null'
    rc, out, _ = run_lambda(nccl_cmd)
    # find will return 1 if there are permission errors or if not found
    if len(out) == 0:
        return None
    if rc != 1 and rc != 0:
...
def get_env_info():
    ...
    return SystemEnv(
        ...
        cuda_compiled_version='This is difficult to identify',
        cuda_runtime_version=get_running_cuda_version(run_lambda),
        nvidia_gpu_models=get_gpu_info(run_lambda),
        nvidia_driver_version=get_nvidia_driver_version(run_lambda),
        cudnn_version=get_cudnn_version(run_lambda),
        nccl_version=get_nccl_version(run_lambda),
...
CUDA used to build TensorFlow: {cuda_compiled_version}
OS: {os}
...
def pretty_str(envinfo):
    ...
    if not is_gpu_available and all_dynamic_cuda_fields_missing:
